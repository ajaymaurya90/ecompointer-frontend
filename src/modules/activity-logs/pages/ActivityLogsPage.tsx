...
import {
    getMyActivityLog,
    getMyActivityLogs,
} from "@/modules/activity-logs/api/activityLogsApi";
...
export default function ActivityLogsPage() {
    const [logs, setLogs] = useState<BrandOwnerActivityLogItem[]>([]);
    const [meta, setMeta] = useState<BrandOwnerActivityLogListResponse["meta"]>({
        page: 1,
        limit: pageSize,
        total: 0,
        totalPages: 1,
    ...
    const [selectedLog, setSelectedLog] =
        useState<BrandOwnerActivityLogItem | null>(null);
    const [detailLoading, setDetailLoading] = useState(false);

    const hasFilters = useMemo(
        () => Boolean(search.trim() || actionKey.trim() || dateFrom || dateTo),
        [search, actionKey, dateFrom, dateTo],
    );
    ...
    async function clearFilters() {
        ...
        try {
            setLoading(true);
            setError(null);
            const response = await getMyActivityLogs({ page: 1, limit: pageSize });
            setLogs(response.data ?? []);
            setMeta(response.meta);
            setPage(response.meta.page);
        } catch (err) {
    ...
    async function openDetail(log: BrandOwnerActivityLogItem) {
        setSelectedLog(log);
        setDetailLoading(true);
        try {
            setSelectedLog(await getMyActivityLog(log.id));
        } catch (err) {
        ...
        <div className="space-y-6">
            <section className="rounded-2xl border border-borderSoft bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 max-w-3xl text-sm text-textSecondary">
                    Review read-only activity for your Brand Owner account, including
                    products, sales channels, service areas, and storefront order events.
                </p>
            </section>
                ...
                <div className="grid gap-4 md:grid-cols-4">
                    <label className="block md:col-span-2">
                        ...
                        <input
                            ...
                            placeholder="Search summary, action, or entity"
                    ...
                    <label className="block">
                        ...
                        <input
                            list="brand-owner-activity-action-keys"
                            value={actionKey}
                            onChange={(event) => setActionKey(event.target.value)}
                            placeholder="PRODUCT_CREATED"
                            className="mt-2 w-full rounded-2xl border border-borderSoft px-4 py-3 text-sm text-textPrimary outline-none transition focus:border-sidebar"
                        />
                        <datalist id="brand-owner-activity-action-keys">
            ...
            <section className="overflow-hidden rounded-2xl border border-borderSoft bg-white shadow-sm">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {loading ? (
                            <tr>
                                <td colSpan={5} className="px-5 py-8 text-center text-textSecondary">
                ...
                <div className="flex items-start justify-between border-b border-borderSoft p-6">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-textSecondary">
                            Read-only activity information for your account.
                        </p>
                    </div>
                    <button
                        type="button"
